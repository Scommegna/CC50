...
main(void)
{
    //Pergunta ao usuario o numero de garrafas para compor a cancao, um loop para solicitar um inteiro e corrigir erros logicos na entrada solicita um inteiro e e gerado um auxiliar para o loop posterior.
    printf("Por favor, me diga quantas garrafas de cerveja ha no muro:\n");
    
    int GarrafasDeCerveja = 0;

    do
    {
        GarrafasDeCerveja = GetInt();
        
        if(GarrafasDeCerveja <= 0)
        {
            printf("Por favor, digite um numero de garrafas superior a 0:\n");
        }

    } while (GarrafasDeCerveja <= 0);
